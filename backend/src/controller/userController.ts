    ...
    try{
        ...
        if(!user){
            ...
            if(isFollowing){
                ...
                currUsersFollowings = currUsersFollowings?.filter((item)=> item._id.toString()!= id)
                usersFollowers = usersFollowers.filter((item)=> item._id!=currUserId)
                const updatedCurrUser = await User.findByIdAndUpdate(currUserId,{followings:currUsersFollowings})
                const updatedUser = await User.findByIdAndUpdate(id,{followers : usersFollowers})
                isFollowing = false
            }
            else{
...
export const getMyFollowings = async(req:Request,res:Response):Promise<void>=>{
    try{
...
export const getMyFollowers = async(req:Request,res:Response):Promise<void>=>{
    try{
        const currUser = await User.findById(req.body.user.id)
